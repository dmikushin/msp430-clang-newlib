...
mkdir -p build && \
cd build && \
cmake -DBUILD_SHARED_LIBS=0 -DCMAKE_BUILD_TYPE=Debug -DLLVM_DEFAULT_TARGET_TRIPLE=msp430-elf -DLLVM_TARGETS_TO_BUILD=MSP430 -DCMAKE_INSTALL_PREFIX=$ROOT/install -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY -DCOMPILER_RT_BAREMETAL_BUILD=TRUE -DCOMPILER_RT_OS_DIR="" .. && \
make -j48 && \
make install
